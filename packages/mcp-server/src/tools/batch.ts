...
async function runBatchTool(
  ...
  // SECOND-LAYER GUARD: did this batch mark a CLAIM invalidated while ways of
  // establishing it are still open? Warning only — a claim can be refuted
  // directly, bypassing its approaches — but it must never pass silently.
  // This is exactly the conflation that killed two hypotheses when only one
  // way of testing each had failed. Read-only, post-commit, and it can never
  // roll anything back.
  let layerWarnings: ReturnType<typeof checkPrematureInvalidation> = [];
  try {
    layerWarnings = checkPrematureInvalidation(
      contextManager.getCurrentProjectId(),
      [...new Set(affectedEdgeIds)],
    );
  } catch {
    // A diagnostic must never break a batch that already committed.
  }
...
Then connect the doc to it with an 'expresses' edge.`;
  ...
  if (layerWarnings.length > 0) {
    const rendered = layerWarnings
      .map((w) => {
        const open = w.openApproaches
          .map((a) => `${a.id} "${a.name}"`)
          .join('; ');
        return `  • ${w.message}\n    still open: ${open}`;
      })
