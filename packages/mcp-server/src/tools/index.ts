...
import { handleLayerTools, layerTools } from './layers.js';
...
export function getToolDefinitions(mode: ToolMode = 'full'): Tool[] {
  ...
  // Second-layer tools (claims vs approaches). Cheap, read-only, and needed
  // in every mode that reads a research graph.
  const coreLayerTools = layerTools;
