    ...
    inputSchema: {
      ...
      properties: {
        trigger: {
          type: 'string',
          description:
            'Trigger type to search for: foundation, surprise, tension, consequence, repetition, question, serendipity, decision, experiment, analysis, library, prediction, evaluation, hypothesis, model, reference',
          enum: [
            'foundation',
            'surprise',
            'tension',
        ...
        unresolvedOnly: {
          type: 'boolean',
          description:
            'Keep only nodes with NO adjudicating edge, for ANY trigger: no validates/invalidates in either direction, no incoming answers (questions), no incoming supersedes. Each returned node also carries a derived `status`. (default: false)',
        },
        missingMetadata: {
          type: 'string',
          description:
...
export async function handleReflectionTools(
  ...
  switch (name) {
    ...
    case 'graph_find_by_trigger': {
      ...
      // With unresolvedOnly the filter can reject most of a page, so fetch the
      // whole trigger class instead of limit*4 — otherwise `returned` silently
      // undercounts the frontier, which is the one number the caller trusts.
      const nodes = queryNodes(projectId, {
        trigger,
        limit: unresolvedOnly ? 100_000 : limit * 4,
        includeArchived,
      });

      // Filter for unresolved if requested (Cold Case Protocol).
      //
      // TRIGGER-AGNOSTIC. The previous version only knew `prediction` and
      // `question`; for every other trigger it silently returned the whole
      // set while still reporting `unresolvedOnly: true`. That silence is
      // worse than an error — a convention built on the flag (e.g. "open
      // approaches" over trigger `experiment`) would have looked filtered
      // and been unfiltered. Status now comes from `deriveStatus`, which is
      // the same rule `graph_approaches` and `graph_frontier` use.
      let filteredNodes = nodes;
      let statusIndex: ReturnType<typeof loadGraphIndex> | undefined;
      const statusById = new Map<string, ReturnType<typeof deriveStatus>>();
      if (unresolvedOnly) {
        statusIndex = loadGraphIndex(projectId, includeArchived);
        filteredNodes = filteredNodes.filter((n) => {
          const res = deriveStatus(
            n.id,
            statusIndex as ReturnType<typeof loadGraphIndex>,
            n.trigger ?? trigger,
          );
          statusById.set(n.id, res);
      ...
      if (unresolvedOnly) {
        hint =
          trigger === 'prediction'
            ...
            : trigger === 'question'
              ? 'These questions have NO answers/validates/invalidates edges - consider answering or marking as abandoned'
              : trigger === 'experiment'
                ? 'These experiments (layer-2 approaches) carry NO validates/invalidates edge in either direction - they are ways of establishing a claim that are still live. graph_approaches shows them under one claim; graph_frontier shows both layers at once.'
                : `These ${trigger} nodes carry no adjudicating edge (validates/invalidates, in either direction) and no incoming supersedes.`;
      } else {
        hint =
          trigger === 'question'
      ...
      return {
        ...
        unresolvedRule: unresolvedOnly
          ? 'no validates/invalidates edge in either direction; for questions also no incoming answers; no incoming supersedes'
          : undefined,
        nodes: results.map((n) => {
          ...
          return {
            id: n.id,
            name: n.title,
            status: statusById.get(n.id)?.status,
            understanding:
