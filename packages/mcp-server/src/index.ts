...
class UnderstandingGraphServer {
  ...
  async start(): Promise<void> {
    ...
    // Report the backend actually in force. The old banner hard-coded
    // "SQLite-only" and kept saying it while the server was demonstrably
    // serving Postgres — which is worse than no banner, because it is the
    // first thing anyone reads when diagnosing a storage problem.
    const backend = (process.env.UG_DB_BACKEND || 'sqlite').toLowerCase() === 'postgres'
      ? 'Postgres'
      : 'SQLite';
    console.error(`Understanding Graph MCP Server v2 running (${backend})`);
  }
