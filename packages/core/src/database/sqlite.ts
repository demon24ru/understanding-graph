...
import { getDbBackend } from './pg/config.js';
import { listPgProjects, openPgDatabase } from './pg/index.js';
...
export function initAllDatabases(projectsDirPath: string): void {
  ...
  const isPostgres = getDbBackend() === 'postgres';
  ...
  // With the Postgres backend a project is a schema, not a store.db file, so
  // the directory listing is no longer the source of truth about which
  // projects exist. The directory is still used, for generated markdown and
  // uploaded documents, so it is created for every project found in the
  // database.
  if (isPostgres) {
    for (const projectId of listPgProjects()) {
      const projectPath = path.join(projectsDirPath, projectId);
...
export function initDatabase(projectPath: string): DatabaseType {
  ...
  // Postgres backend: the project's schema replaces store.db entirely. It is
  // created and migrated by openPgDatabase, so none of the SQLite DDL and
  // ALTER-based migrations below apply — the Postgres schema is authored in
  // its final shape in database/pg/schema.ts.
  if (getDbBackend() === 'postgres') {
    const pgDb = openPgDatabase(projectId) as unknown as DatabaseType;
    databases.set(projectId, pgDb);
    currentProjectId = projectId;
    return pgDb;
  }
