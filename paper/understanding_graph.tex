...
We name this architecture the ``Understanding Graph'' to mark a deliberate distinction from Knowledge Graphs. A Knowledge Graph stores the products of cognition: entities, relations, summaries. An Understanding Graph stores the \emph{process} of cognition: the path from not-knowing to knowing. Most existing AI memory systems---whether they call themselves knowledge graphs, RAG pipelines, or agent memories---store products; the lineages noted in §\ref{sec:reasoning-capture} that begin to persist reasoning itself (Reflexion, Buffer of Thoughts, A-MEM, Hindsight, Graphiti) refine reasoning into more compact artifacts before storing it. This paper stores the process.
...
\section{The Reasoning-Capture Paradigm}\label{sec:reasoning-capture}
...
We employ a ``Decoupled State'' architecture where the \emph{Graph} stores the logical structure of beliefs (state), distinct from the \emph{LLM} which generates the prose (renderer). The remainder of this section walks through that state from the bottom up: first the typed taxonomy of nodes and edges that gives the graph its shape (§\ref{sec:node-types}), then the metabolic memory model that lets it evolve (Metabolic Memory and the Cognitive Ratchet), then the formal supersession mechanism that makes belief revision auditable (§\ref{sec:formal-model}), and finally the retrieval and synthesis machinery that lets agents work with graphs larger than any single context window.
...
Unlike textual diffs, the supersession edge records a \emph{Semantic Diff} in its \texttt{why} field (e.g., \texttt{"why": "Section 4.2 showed expressivity loss..."}), creating a machine-readable changelog. This mechanism operationalizes parts of the AGM bookkeeping for belief revision~\cite{alchourron1985agm}---specifically, properties analogous to \emph{Success} (new beliefs are always added), \emph{Inclusion} (the new belief set contains the revision), and \emph{Recovery} (old beliefs remain accessible via the supersession chain). It does not implement a full AGM revision operator: minimality of change (\emph{Vacuity}) and consistency restoration are not algorithmically enforced; the supersession edge records the human or agent decision rather than computing a unique minimal-revision result. Recent empirical work demonstrates that LLMs struggle with belief revision when presented with contradictory evidence, often failing to update prior conclusions~\cite{xu2024belief}; the Understanding Graph addresses this by making revision an explicit, typed graph operation rather than relying on the model's implicit ability to override prior context.
...
The multi-agent configuration multiplies token cost without proportionally inflating wall-clock latency: when agents can be dispatched against disjoint graph regions of the shared substrate, reading time remains close to single-agent. The parallelism advantage degrades when their work overlaps---contention on the same subgraph forces serialization---or when downstream synthesis must wait for the slowest worker; the architecture amortizes parallelism rather than guaranteeing it. What the extra tokens buy is \emph{separability}---a single prompt instructing one model to ``think like an axiologist, then a psychologist, then a skeptic'' collapses under role-confusion, because nothing in its context distinguishes which hat is speaking. Running the roles as distinct agents stamps each contribution with its provenance, and inter-agent disagreements become first-class \texttt{Tension} nodes rather than contradictions the model silently averages away.

\subsection{Coordination Through Stigmergy}

Coordination happens through the graph itself rather than through inter-agent messaging. Agents read \texttt{graph\_history()} to see what others have written, signal intent and completion by writing into the shared topology, and resolve disagreements by minting \texttt{Tension} or \texttt{diverse\_from} edges rather than by message-exchange. The graph is both the memory layer and the coordination layer; no separate task-queue or central dispatcher is required. (Earlier iterations included a dedicated task queue with advisory locking; we found these added a coordination substrate parallel to the graph without offering coordination affordances the graph itself does not already provide, and removed them in favor of pure stigmergic coordination.)
For high-contention deployments, explicit advisory locks or subtree leases may still be useful as an optional scaling layer; the present implementation treats graph history and typed conflict edges as the primary coordination mechanism.
...
This mode represents the system's most distinctive capability: generating structurally grounded reasoning traces for training future language models.

\subsubsection{The Reading Protocol and Latent Space Steering}
\label{sec:reading-protocol}

The agent consumes source text in chronological order, pausing after each section to record its cognitive state. The system supports \emph{Temporal Integrity} through an append-only architecture designed for synthetic data generation. The \texttt{source\_read} tool streams text in chunks, while \texttt{doc\_append\_thinking} inserts reasoning nodes strictly at the end of the current chain. This architectural constraint prevents \emph{tool-supplied} future text from entering the agent's context---no later chunk is delivered until the current chunk's thinking nodes are committed---so the trace's \emph{structural} chronology is enforced. Parametric memory and inference-time prediction are not blocked: the model's weights may already encode what comes later, and the discipline of the resulting trace depends on the agent honoring the chronological framing rather than on the harness making future-knowledge inaccessible. This trade-off mirrors the Epistemic Horizon discussion in~\cite{westerberg2026entangled}.
...
This format trains models to pause and reflect, connect new information to existing structures, notice tensions, and maintain epistemic humility. Structural quality is monitored via the \texttt{graph\_score} function, which evaluates structural properties such as connectivity and orphan rates. It penalizes ``orphan'' thinking nodes that lack connections to at least two concept nodes and rewards high supersession rates and the eventual resolution of question nodes, targeting a score of $\geq 70$. \texttt{graph\_score} is a structural proxy, not a semantic correctness check (see §\ref{sec:limitations}); a structurally clean graph can still be semantically nonsense.
...
Interaction with the graph is mediated through the Model Context Protocol (MCP)~\cite{anthropic2024mcp}. This integration exposes specialized tools through role-scoped registries: a full registry for administration and narrower reading and research registries for task-specific work. The tools cover graph mutation, query, analysis, document handling, and serendipity. Table~2 summarizes the primary categories:
...
\subsection{Safety and Orchestration}\label{sec:safety-orchestration}
...
We govern the exploration-exploitation trade-off via a \emph{Graph Thermostat}. This meta-cognitive mechanism calculates the entropy of the graph by measuring the relative density of \texttt{Tension} nodes (conflict) and \texttt{Orphan} nodes (fragmentation). It dynamically selects among three strategies: \texttt{DIVERGE} (injecting chaos when the graph is stagnant), \texttt{CONVERGE} (consolidating insights when the graph is fragmented), or \texttt{MAINTAIN} (continuing the current approach when entropy is balanced). This resembles the criticality intuition in the entropic-brain framework~\cite{carhartharris2014entropic}: useful cognition appears to require neither rigid order nor unconstrained chaos, but movement near a productive boundary between stability and flexibility. The analogy is structural rather than mechanistic---Carhart-Harris is talking about brain-state entropy, while the Graph Thermostat operates on graph-structural entropy heuristics---but the same control intuition applies.

The system encourages synthesis quality through a \emph{PURE Evaluation} protocol~\cite{westerberg2026ri}. Each synthesis is assessed across four gates: \emph{Parsimonious} (minimal hidden prerequisites), \emph{Unique} (makes predictions other theories do not), \emph{Realizable} (specifies a coherent mechanism), and \emph{Expansive} (transfers to at least one other domain). Each gate receives a RED/YELLOW/GREEN rating with steelmanned arguments for and against. The decision rule is non-compensatory: a synthesis may proceed if and only if no gate is RED. For YELLOW gates, the agent is asked to identify the smallest intervention likely to advance it to GREEN. PURE is currently implemented primarily as prompt-level guidance and graph quality tooling (skills, scoring tools, and instruction layers), with experimental solver-side logging support where that tool surface is enabled. It is not yet an always-on enforcement layer: agents can bypass PURE unless a workflow explicitly routes synthesis through it. The next architectural step is to promote these soft constraints into a mandatory verifier and audit log, moving from coaching tier to enforcement tier.

Beyond PURE evaluation, a planned \emph{Active Coaching Layer} would enforce structural constraints through a ``Safety Interceptor'' middleware that inspects tool calls before execution: when an agent attempts a forbidden pattern, the interceptor would reject the call with a corrective pedagogical message, turning runtime errors into learning signals for the agent. This sits at the same enforcement tier as the verifier described above and is similarly future work; the current implementation relies on prompt-level coaching and the role-scoped tool registries described in §6.1 to discourage forbidden patterns rather than on runtime interception.
...
Mark Burgess's Semantic Spacetime framework offers an independent precursor to this view of graph-based understanding. In \emph{Spacetimes with Semantics III}~\cite{burgess2016spacetimes}, Burgess frames knowledge representation as a semantic spacetime problem, emphasizing scale separation, recurring association types, identity discrimination, and memory as requirements for artificial reasoning. His SSTorytime project~\cite{burgess_sstorytime} applies this lineage to story-shaped knowledge maps that preserve processes, provenance, and trajectories rather than only static entities. The Understanding Graph arrives from the LLM side rather than from semantic-process theory, but the convergence is useful: both reject fact-only memory in favor of typed movement through relations.
...
A related risk of metabolic memory is premature convergence---if an early incorrect belief is strongly connected, it may resist supersession. This resembles \emph{canalization} in predictive-processing accounts~\cite{carhartharris2014entropic}, where patterns of inference can become overly grooved and resistant to revision; in our case the grooves are explicit graph topology rather than implicit precision-weighting, but the failure mode rhymes. While Temporal Attention Decay mitigates this by freezing unused paths, active error-correction remains a challenge.
...
A further class of limitations sits at the implementation tier rather than the architectural one. The Cognitive Guardrail, the PURE evaluation protocol (§\ref{sec:safety-orchestration}), and the Refraction discipline of identity-anchored thinking are currently realized as prompt-level coaching---skill prompts, tool-layer hints, and graph quality metrics---rather than as mandatory runtime gates. An agent that ignores the coaching can still write to the graph; overuse of generic edges or under-typed nodes is visible after the fact rather than blocked at write-time. The architectural target is a hard verifier with an enforcement log. The current substrate operates one tier below this, and adjacent features such as \texttt{source\_read}-driven chronological ingestion inherit the same coaching-tier contract.
...
A companion paper~\cite{westerberg2026entangled} exercises the architecture on two domains at opposite poles of the cognitive spectrum: high-entropy literary narrative (Kafka's \emph{Metamorphosis}, producing a 346-node graph with graph health 89/100) and low-entropy technical theory (LLaDA~\cite{nie2025llada}, a paper published after the generation model's documented knowledge cutoff, producing 285 nodes at 80/100). Both runs achieved 100\% structural traceability---every generated node has a provenance edge to source evidence (this is enforced as a hard pipeline constraint rather than measured as an emergent property). The graph topology shifted with domain: narrative reading allocated 24.3\% of nodes to \texttt{Tension} (psychological conflict), while technical reading shifted density toward \texttt{Analysis} (23.9\%) and increased \texttt{Evaluation} density by nearly 50\% (6.6\% to 9.8\%). The two runs used hand-configured agent rosters (Critic and Psychologist for narrative; Architect and Methodologist for technical), so the topology shift cannot be cleanly attributed to the architecture itself absent a roster-neutral comparison; the shift is consistent with domain-adaptive cognitive allocation, but the cause-attribution is left as an open ablation. On the LLaDA paper---material released after the generation model's documented cutoff (we did not run a pre-read probe, so this is a contamination-reducing setup rather than a clean check)---the swarm surfaced architectural framings (bidirectional reasoning as ``epistemic grace,'' inference latency as a ``contemplation tax'') that align with the paper's theoretical commitments without being primed by the prompt. These results are consistent with structural validity and domain-conditioned topology; the deeper question---whether the captured traces constitute genuine evaluative depth rather than formulaic pattern-completion---is deferred to the human-baseline comparisons proposed in that paper's experimental roadmap. Future work will pair \texttt{graph\_score} with downstream benchmark performance---e.g., multi-hop question answering and logic-puzzle solving against a vanilla RAG baseline---to test whether structural graph health actually predicts semantic utility.

\subsection{Potential Applications}

The architecture is intended as cognitive infrastructure for domains where knowledge evolves through revision. The application sketches below are conceptual rather than empirically validated deployments; each describes what the architecture's properties would enable if successfully applied to the domain.
...
The deeper claim is ontological. When cognition happens in tokens, understanding becomes directly storable in the medium where it occurs. The Understanding Graph is the architecture that becomes possible on the other side of that transition.
...
\bibitem{burgess2016spacetimes}
Burgess, M. (2016).
Spacetimes with Semantics (III): The Structure of Functional Knowledge Representation and Artificial Reasoning.
\textit{arXiv:1608.02193}. \url{https://doi.org/10.48550/arXiv.1608.02193}.

\bibitem{burgess_sstorytime}
Burgess, M. (2025).
SSTorytime: Semantic Spacetime Story Graph Database.
Software, NLnet ``Smart Lookup \& Inference for Semantic Data'' project.
\url{https://github.com/markburgess/SSTorytime}.

\bibitem{carhartharris2014entropic}
